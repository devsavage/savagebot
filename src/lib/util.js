...
module.exports = {
    ...
    removeHash: function removeHash(string) {
        return ltrim(string.toLowerCase(), "#");
    },
    ...
    isBlacklisted: function(username) {
        return this.inArray(username, config.bot.settings.blacklist);
    },
    ...
    getFollowAge: function getFollowAge(username, followDate) {
        var toDate = new Date();
        var levels = 3;
        var prefix = null;
    ...
    isValidJson: function isValidJson(string) {
    	try {
    		JSON.parse(string);
    	} catch(err) {
    		return false
